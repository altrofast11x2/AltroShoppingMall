...
const ADMIN_SEED_COINS = 100000000; // 관리자 테스트용 초기 잔액 (실제 차감됨)

// 관리자 레코드를 shop_users/admin 에 보장 — 코인이 실제로 차감/저장되도록
async function ensureAdminRecord() {
  try {
    const snap = await get(ref(db, 'shop_users/admin'));
    if (snap.exists()) {
      const u = snap.val();
      return typeof u.coins === 'number' ? u.coins : ADMIN_SEED_COINS;
    }
    await set(ref(db, 'shop_users/admin'), {
      name: '관리자', email: ADMIN_EMAIL, isAdmin: true,
      coins: ADMIN_SEED_COINS, createdAt: new Date().toISOString(),
    });
    return ADMIN_SEED_COINS;
  } catch (e) {
    console.warn('[AltroShop] 관리자 레코드 보장 실패:', e?.message || e);
    return ADMIN_SEED_COINS;
  }
}
...
export async function loginUser(email, password) {
  ...
  if (lower === ADMIN_EMAIL && password === ADMIN_PASSWORD) {
    const coins = await ensureAdminRecord();
    return { id: 'admin', name: '관리자', email: lower, isAdmin: true, coins };
  }
...
export async function getUser(uid) {
  if (uid === 'admin') {
    try {
      ...
      if (snap.exists()) {
        const u = snap.val();
        return { id: 'admin', name: '관리자', email: ADMIN_EMAIL, isAdmin: true, coins: typeof u.coins === 'number' ? u.coins : ADMIN_SEED_COINS };
      }
    } catch {}
    return { id: 'admin', name: '관리자', email: ADMIN_EMAIL, isAdmin: true, coins: ADMIN_SEED_COINS };
  }
  try {
...
export async function updateUserCoins(uid, coins) {
  // 관리자도 shop_users/admin 에 실제 저장 → 구매 시 코인이 눈에 보이게 차감됨
  await update(ref(db, `shop_users/${uid}`), { coins: Math.max(0, Number(coins) || 0) });
  return true;
}

// ───────── PRODUCTS ─────────
export async function createProduct({ sellerId, sellerName, name, desc, price, image, category }) {
...
// 상품 상세에서 "바로 구매" — 단일 상품 즉시 안전결제 (코인 즉시 차감)
export async function checkoutDirect(uid, productId, qty = 1) {
  const user = await getUser(uid);
  if (!user) return { error: '로그인이 필요합니다' };
  const p = await getProduct(productId);
  if (!p) return { error: '상품을 찾을 수 없습니다' };
  if (p.sellerId === uid) return { error: '본인 상품은 구매할 수 없습니다' };

  const total = Number(p.price) * qty;
  if ((user.coins || 0) < total) {
    return { error: `코인이 부족합니다 (필요: ${total.toLocaleString()}, 보유: ${(user.coins || 0).toLocaleString()})` };
  }

  // 구매자 코인 즉시 차감 (에스크로 보관)
  await updateUserCoins(uid, (user.coins || 0) - total);
...
// ───────── REPORTS (문제 신고 — AltroShop 내부) ─────────
// shop_reports/{id}: { reporterId, reporterName, targetType, targetId, targetName, reason, detail, status, createdAt }
export async function createReport({ reporterId, reporterName, targetType, targetId, targetName, reason, detail }) {
  const newRef = push(ref(db, 'shop_reports'));
  const r = {
...
export async function listReports() {
  try {
    const snap = await get(ref(db, 'shop_reports'));
    if (!snap.exists()) return [];
...
export async function resolveReport(reportId) {
  await update(ref(db, `shop_reports/${reportId}`), {
    status: 'resolved',
    resolvedAt: new Date().toISOString(),
  });
  return { ok: true };
