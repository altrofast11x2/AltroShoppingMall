...
const I: any = {
  ...
  Shop: (p: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>,
  ...
  Coin: (p: any) => <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" {...p}><circle cx="12" cy="12" r="9"/><text x="12" y="16" textAnchor="middle" fontSize="10" fill="currentColor" stroke="none" fontWeight="700">◈</text></svg>,
...
const OTHER_APPS = [
  { label: 'AltroBoard',  href: 'https://altroboard.vercel.app/',       external: true },
  { label: '게시판',       href: 'https://altroboard.vercel.app/board',  external: true },
  { label: '갤러리',       href: 'https://altroboard.vercel.app/galleries', external: true },
  { label: '게임',         href: 'https://altroboard.vercel.app/games',  external: true },
];
...
export default function NavBar() {
  ...
  useEffect(() => {
    setHover(false); setMobileOpen(false); setAppsOpen(false);
  }, [pathname]);
  ...
  return (
    <>
        ...
        <nav className="sb-nav">
          ...
          {user && (
            <div className="sb-coin-row" title={`${Number(user.coins || 0).toLocaleString()} 코인`}>
              <span className="sb-icon" style={{color:'var(--gold)'}}><I.Coin width={22} height={22}/></span>
            ...
            <div className="sb-sublist">
              {OTHER_APPS.map(a => (
                <a key={a.href} href={a.href} target="_blank" rel="noopener noreferrer" className="sb-row sb-sub">
                  <span className="sb-icon"><I.External width={18} height={18}/></span>
        ...
        <div className="sb-bottom">
          {user ? (
            <>
              ...
              <button className="sb-row sb-btn" onClick={toggleTheme}>
                <span className="sb-icon"><I.Moon width={22} height={22}/></span>
                <span className="sb-label">테마 전환</span>
              </button>
      ...
      {/* 본문 자리 확보 (호버해도 안 밀림 — collapsed 너비 고정) */}
      ...
      <style>{`
        ...
        .sb-other-apps{border-top:1px solid rgba(255,255,255,.06);padding:.3rem 0;}
